...
if (isset($_POST['attend_form'])) {
	...
	$check = $wpdb->get_results("SELECT * FROM wp_screenings_attending WHERE email='$email' AND message='$msg'");
	...
	if ($count < 1) {
		...
		$rows_affected = $wpdb->insert('wp_screenings_attending', array( 
			'wp_screenings_events_id' => $itemID,
			'email' => $email,
			'name' => $name,
			'message' => $msg,
			'attendants' => $number
	...
	<p>'.$name.' is awaiting your reply, please reply directly to '.$email.'.<br />
		...
		<p>If your Screening is fully booked please login and change status on <a href="http://petitesplanetes.cc/screenings/manage/">http://petitesplanetes.cc/screenings/manage/</a></p>
	<p>Best Wishes,</p>
	<p>Vincent Moon<br />
	...
	$(document).ready(function() {
		...
		$("#attent_screening").submit(function() {
	...
	<div id="attent_screening">
		<p>This message will be sent to the host of the Screening at <b>'.$screening->place.'</b> in '.$screening->city.'.<br /></p>
	<form class="validate" id="attent_screening" name="attent_screening" method="post"action="'.str_replace("%7E", "~", $_SERVER["REQUEST_URI"]).'">
	<p>
		<input type="hidden"name="attend_form"value="true">
		<input type="hidden" name="screening_id" value="'.$itemID.'">
